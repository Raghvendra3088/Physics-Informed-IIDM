...
# ── Paths ──────────────────────────────────────────────────────────────────────
ROOT      = Path("/DATA1/anil/Implementation-of-IIDM")
OUT_ROOT  = Path("/DATA1/anil/Physics-Informed-IIDM")
...
def main():
    ...
    with rasterio.open(MASKS_DIR / "forest_mask.tif") as src:
        mask_arr = (src.read(1) == 1).astype(np.float32)

    # 6 channels: Red (B4), Green (B3), Blue (B2), NIR (B8), DEM, Canopy
    input_stack = np.concatenate([b4_arr, b3_arr, b2_arr, b8_arr, dem_arr, canopy_arr], axis=0)
    target = carbon_arr
